...
能量是有限的。计算有代价。DeepSeek 不堆参数，堆算法。本项目以同样的工程哲学落地了四件事：

**熵预算** — 在 research-orchestrator 中，每个管线阶段标注了激活条件。博士课题走全管线；日常查询只跑单步搜索。计算资源按问题重要性分配，不浪费。

**稀疏激活** — karpathy-guard 定义了 MoE 式路由规则：规划器必跑（轻量），搜索器有词才跑，分析器有结果才跑，写手有发现才跑。每个模块像一个静默的神经元——只在信号到达阈值时点火。

**差分更新** — verify-stats-handbook 不再做全量检查。概率陈旧评分 P(stale) 只计算有变更的包，复查周期由更新频率、破坏性变更概率和用户依赖度联合决定——而不是固定的 3 个月。

**信息增益路由** — ima-smart-search 按信息增益排关键词。P0 精确术语（如 glmmTMB）先搜，命中即停。P2 冗余词（如"分析方法"）跳过。搜到相同的文件跨库去重。
...
AI执行：搜索、分析、生成、标记涌现、建议修订等。
人执行：判断真伪、选择方法、决定方向、承担发表责任等。
