...
class TheoryGraph:
    ...
    def __init__(self, graph_path=None):
        if graph_path is None:
            ...
            graph_path = base / "config" / "knowledge_base" / "ecological_theories" / "_topology" / "theory_tree_graph.json"
            if not graph_path.exists():
                graph_path = Path("config/knowledge_base/ecological_theories/_topology/theory_tree_graph.json")
    ...
    def _resolve_id(self, short_id):
        ...
        return short_id  # 返回原始值，后续会报 KeyError(self, node):
        """从理论节点提取特征词"""
